...
os.environ["OPENAI_API_KEY"] = "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"
os.environ["ANTHROPIC_API_KEY"] = "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"
os.environ["PERPLEXITY_API_KEY"] = "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"
...
# Create separate LLM configurations
gpt4_llm = ChatOpenAI(
    model="gpt-4o-mini",
    temperature=0.7
)
...
def chat():
    ...
    overview_crew.kickoff()
    overview_response_raw = format_task.output.raw
    overview_cleaned = overview_response_raw.strip('```html').strip('```').strip()
    overview_response_formatted = escape_special_characters(overview_cleaned)

    # scopes_crew.kickoff()
    # scopes_response_raw = format_task.output.raw
    # scopes_cleaned = scopes_response_raw.strip('```html').strip('```').strip()
    # scopes_response_formatted = escape_special_characters(scopes_cleaned)
    
    # tools_partners_crew.kickoff()
    # tools_partners_response_raw = format_task.output.raw
    # tools_partners_cleaned = tools_partners_response_raw.strip('```html').strip('```').strip()
    # tools_partners_response_formatted = escape_special_characters(tools_partners_cleaned)

    # reporting_crew.kickoff()
    # reporting_response_raw = format_task.output.raw
    # reporting_cleaned = reporting_response_raw.strip('```html').strip('```').strip()
    # reporting_response_formatted = escape_special_characters(reporting_cleaned)

    # compliance_crew.kickoff()
    # compliance_response_raw = format_task.output.raw
    # compliance_cleaned = compliance_response_raw.strip('```html').strip('```').strip()
    # compliance_response_formatted = escape_special_characters(compliance_cleaned)

    # targets_crew.kickoff()
    # targets_response_raw = format_task.output.raw
    # targets_cleaned = targets_response_raw.strip('```html').strip('```').strip()
    # targets_response_formatted = escape_special_characters(targets_cleaned)

    # decarbonization_crew.kickoff()
    # decarbonization_response_raw = format_task.output.raw
    # decarbonization_cleaned = decarbonization_response_raw.strip('```html').strip('```').strip()
    # decarbonization_response_formatted = escape_special_characters(decarbonization_cleaned)
    ...
    return render_template(
        ...
        # Add other responses here as you create additional crews
    )
